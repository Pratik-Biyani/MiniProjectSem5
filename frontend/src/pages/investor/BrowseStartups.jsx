...
const BrowseStartupsPage = () => {
  ...
  // Fetch startups from your API - same endpoint as investors but filter for startup role
  const fetchStartups = async () => {
    try {
      ...
      if (result.success) {
        // Filter users to only show startups
        const startupUsers = result.data.users.filter(user => user.role === 'startup');
  ...
  const filteredStartups = startups.filter(startup => {
    ...
    const matchesDomain = !filters.domain || startup.domain?.toLowerCase().includes(filters.domain.toLowerCase());
    const matchesSubscription = !filters.subscriptionStatus || 
                               (filters.subscriptionStatus === 'subscribed' && startup.isSubscribed) ||
                               (filters.subscriptionStatus === 'unsubscribed' && !startup.isSubscribed);

    return matchesSearch && matchesDomain && matchesSubscription;
  });

  // Get unique domains for filter dropdown
  const domains = [...new Set(startups.map(startup => startup.domain).filter(Boolean))];
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search startups by name, email, or domain..."
                value={searchTerm}
...
const StartupCard = ({ startup, onConnect, onVideoCall }) => {
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-gray-400">
                Joined {new Date(startup.createdAt).toLocaleDateString()}
              </span>
            </div>
          </div>
        </div>
          ...
          <>
            <div className="border-t pt-3 mt-3 space-y-2">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-gray-900 capitalize">{startup.role}</span>
              </div>
              {startup.domain && (
                <div className="flex justify-between items-center text-sm">
                  <span className="font-medium text-gray-700">Industry Focus:</span>
                  <span className="text-gray-900">{startup.domain}</span>
                </div>
              )}
            </div>
          </>
